...
async function load(){try{$('status').textContent='Loading actual AKS-74U pack…';result=await BABYLON.SceneLoader.ImportMeshAsync('','assets/','aks74u_ue_actual_arms_authored.glb',scene);root=topRoot(result);fillAnimations();resetRig();$('status').textContent='ACTUAL AKS-74U + AUTHORED UE ANIMS';log(`Loaded ${result.meshes.length} meshes, ${(result.skeletons||[]).length} skins/skeletons, ${(result.animationGroups||[]).length} authored animation groups.`);preset('reload');setTimeout(fit,120)}catch(e){$('status').textContent='LOAD ERROR';log(e.stack||e.message||String(e));console.error(e)}}
